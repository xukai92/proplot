...
def test_ignored_keywords():
    ...
    fig, ax = pplt.subplots(
        gridspec_kw={'left': 3}, subplot_kw={'proj': 'cart'}, subplotpars={'left': 0.2}
    )
    fig.subplots_adjust(left=0.2)


def test_init_formatting():
    """
    Test basic application format args on initialization. Previously patch keywords
    """
    # would fail.
    ...
    fig = pplt.figure(refaspect=2, facecolor='blue1')
    fig.subplots(ncols=2, hatch='xxx', hatchcolor='k', facecolor='blue3')
...
def test_spine_side():
    ...
    """
    fig, ax = pplt.subplots()
    ax.plot(pplt.arange(-5, 5), (10 * np.random.rand(11, 5) - 5).cumsum(axis=0))
...
def test_colormap_assignment():
    """
    Test below line is possible.
    """
